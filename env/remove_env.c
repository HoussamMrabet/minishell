...
/*   Updated: 2024/05/12 08:11:36 by hmrabet          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	remove_fake_env(t_minishell *minishell, char *var)
{
	...
	(1) && (j = 0, i = 0);
	while (minishell->fake_env[i] && !(starts_with(var, minishell->fake_env[i])))
		i++;
	...
	(1) && (new_env = ft_malloc(&minishell->global, sizeof(char *) * (j + 1)), j = 0, i = 0);
	while (minishell->fake_env[i])
	{
		if (starts_with(var, minishell->fake_env[i]))
		{
